...
def upgrade():
    ...
    for warehouse in [267]:
        sql = """insert into settings (value, target_type, target_id) values ('{0}', 'warehouse', {1})""".format(settings_json_denton, warehouse)
        op.execute(sql)
    ...
    for warehouse in [241]:
        sql = """insert into settings (value, target_type, target_id) values ('{0}', 'warehouse', {1})""".format(settings_json_glcc1, warehouse)
        op.execute(sql)
    ...
    for warehouse in [242]:
        sql = """insert into settings (value, target_type, target_id) values ('{0}', 'warehouse', {1})""".format(settings_json_glcc2, warehouse)
        op.execute(sql)
    ...
    for warehouse in [82, 98, 99, 100, 101, 102, 103, 104, 156, 162, 166, 167, 176, 177, 178, 183, 184, 185, 186, 187]:
        sql = """insert into settings (value, target_type, target_id) values ('{0}', 'warehouse', {1})""".format(settings_json_wm, warehouse)
        op.execute(sql)
