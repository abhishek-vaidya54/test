...
class EmailSchedule(db.Model):
    ...
    group = db.relationship(
        "Group", foreign_keys=group_id, backref="email_schedule_tests"
    )

    shift_id = db.Column(db.Integer, ForeignKey("shifts.id"), nullable=True)
    shift = db.relationship(
        "Shifts", foreign_keys=shift_id, backref="email_schedule_tests"
    )
    ...
    job_function = db.relationship(
        "JobFunction", foreign_keys=job_function_id, backref="email_schedule_tests"
    )

    warehouse_id = db.Column(db.Integer, ForeignKey("warehouse.id"), nullable=False)
    warehouse = db.relationship(
        "Warehouse", foreign_keys=warehouse_id, backref="email_schedules"
    )

    client_id = db.Column(db.Integer, ForeignKey("client.id"), nullable=False)
    client = db.relationship(
        "Client", foreign_keys=client_id, backref="email_schedules"
    )
