...
class IndustrialAthlete(db.Model):
    ...
    client = db.relationship("Client", foreign_keys=client_id, backref="athletes")
    ...
    hire_date = db.Column(db.DateTime, default=datetime.date.today(), nullable=True)

    termination_date = db.Column(db.DateTime, nullable=True)

    warehouse_id = db.Column(db.Integer, ForeignKey("warehouse.id"), nullable=True)
    warehouse = db.relationship(
        "Warehouse", foreign_keys=warehouse_id, backref="athletes"
    )
    shift_id = db.Column(db.Integer, ForeignKey("shifts.id"), nullable=True)
    shift = db.relationship("Shifts", foreign_keys=shift_id, backref="athletes")
    job_function_id = db.Column(
        db.Integer, ForeignKey("job_function.id"), nullable=True
    )
    job_function = db.relationship(
        "JobFunction", foreign_keys=job_function_id, backref="athletes"
    )
    ...
    def get_warehouse_id(self, athlete_id):
        return (
            db.session.query(IndustrialAthlete.warehouse_id)
            .filter(IndustrialAthlete.id == athlete_id)
            .scalar()
        )
...
def get(athlete_id):
    return (
        db.session.query(IndustrialAthlete)
        .filter(
