...
metric_dict = {
    ...
    'max-lateral-velocity': sa.func.avg(db.Risk.max_lateral_velocity).label('Max Lateral Velocity'),
    'avg-twist-velocity': sa.func.avg(db.Risk.avg_twist_velocity).label('Average Twist Velocity'),
}
...
def get_athlete_query(athlete_ids, metric):
    query = db.db.session.query(
        ...
        db.IndustrialAthlete.id.in_(athlete_ids if athlete_ids else [])
    ).group_by(
...
def athlete_average_by_rel_day(athlete_ids, metric='safety-score'):
    ...
    for athlete in query:
        athlete_data = data.setdefault(
            ...
            {'flx': [],
             'no_flx': [],
             'name': athlete.name, 'athlete_id': athlete.id})
