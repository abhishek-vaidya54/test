...
def get_athlete_query(client_guid):
    query = db.db.session.query(
        sa.func.count(sa.func.distinct(sa.func.DATE(db.Risk.start_time))).label('days_of_data'),
...
def athletes(client_guid):
    ...
    db_athletes_by_id = {ia.id: ia for ia in db.db.session().query(
        db.IndustrialAthlete).join(db.Client).filter(db.Client.guid == client_guid)}
    athletes_by_id = {}
