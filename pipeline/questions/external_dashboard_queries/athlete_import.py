...
def valid_columns(content, import_record):
    ...
    for key in rowKeys:
        ...
        try:
            ...
            msg = (
                "There are extra columns found in header row of csv remove them please "
            )
            insert_error(1, import_record, msg)
        i += 1
...
def is_shift(row, row_number, import_record, warehouse):
    if "shift" in row and row["shift"] != "":
        ...
        query1 = apply_warehouse_filter(
            query1, warehouse, column=db.Shifts.warehouse_id
        )
        shift = query1.first()
        if not shift:
...
def is_job_function(row, row_number, import_record, warehouse):
    if "job_function" in row and row["job_function"] != "":
        query2 = db.db.session.query(db.JobFunction).filter(
            db.JobFunction.name == row["job_function"]
        )
        query2 = apply_warehouse_filter(
            query2, warehouse, column=db.JobFunction.warehouse_id
        )
...
def insert_process(row, row_number, client, warehouse, import_record, user_id):
    ...
    if shift and job_function and valid:
        try:
            ...
            # logging.info(exp)
            msg = "Insertion failed column {} does not exist in the records".format(
                exp.message
            )
            insert_error(row_number, import_record, msg)
...
def insert_error(row_number, import_record, msg):
    importerror = ImportErrors(
        row_number=row_number, import_record_id=import_record, error_info=msg
    )
    s2.add(importerror)
...
def db_validation(row, row_number, import_record):
    ...
    if (
        "first_name" in row
        and row["first_name"] != ""
        and re.findall("[^A-Za-z ]", str(row["first_name"]))
    ):
    ...
    if (
        "last_name" in row
        and row["last_name"] != ""
        and re.findall("[^A-Za-z ]", str(row["last_name"]))
    ):
    ...
    if (
        "hire_date" in row
        and row["hire_date"] != ""
        and not validate_date(row["hire_date"])
    ):
    ...
    if (
        "hire_date" not in row
        or "gender" not in row
        or "prior_back_injuries" not in row
        or "height" not in row
        or "weight" not in row
        or "first_name" not in row
        or "last_name" not in row
        or "external_id" not in row
    ):
...
def get_errors(id):
    ...
    for row in data:
        import_error = {"row_num": row.row_number, "error_info": row.error_info}
