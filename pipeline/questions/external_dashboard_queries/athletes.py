...
def get_athlete_query(client_id=None, athlete_id=None, range_start=None, range_end=None, warehouse_id=None, athletes=None, csv_query=False):
...
def _calculate_score(field, last_score = None, current_score = None):
    ...
    first_score = last_score[field]*(round(last_hour/(last_hour + cur_hour), Config.ROUNDING))
    second_score = current_score[field]*(round(cur_hour/(cur_hour + last_hour), Config.ROUNDING))
    
    return round(first_score + second_score, Config.ROUNDING)


def all_athletes_data_shifts(client_id, range_start=None, range_end=None, warehouse_id=None, athlete_ids=None):
    shifts = db.db.session.query(db.Shifts).join(db.Warehouse).filter(db.Warehouse.client_id == client_id).all()
...
def individual_athletes_data(client_id, range_start=None, range_end=None, warehouse_id=None, athlete_ids=None):
    ...
    for athlete in data:
        default = {
            ...
            "hire_date": athlete.hire_date.strftime('%Y-%m-%d') if athlete.hire_date else None,
            "last_modified_date": athlete.last_modified_date.strftime('%Y-%m-%d %H:%M:%S') if athlete.hire_date else None,
        ...
        athlete_value = {
            ...
            "max_lateral_velocity": round(athlete.max_lateral_velocity, Config.ROUNDING),
        ...
        if last_score and last_score['day'] == athlete_value['day']:
            ...
            athlete_value['safety_score'] = _calculate_score('safety_score', last_score, athlete_value)
            athlete_value['max_moment'] = _calculate_score('max_moment', last_score, athlete_value)
            athlete_value['max_flexion'] = _calculate_score('max_flexion', last_score, athlete_value)
            athlete_value['avg_twist_velocity'] = _calculate_score('avg_twist_velocity', last_score, athlete_value)
            athlete_value['max_lateral_velocity'] = _calculate_score('max_lateral_velocity', last_score, athlete_value)
            athlete_value['lift_rate'] = _calculate_score('lift_rate', last_score, athlete_value)
        ...
        athletes_by_id.setdefault(
            ...
            athletes_by_id.get(
                athlete_id,
                {
                    ...
                    "name": athlete["name"],
                    "hire_date": athlete["hire_date"].strftime('%Y-%m-%d') if athlete["hire_date"] else None,
                    "last_modified_date": athlete["last_modified_date"].strftime('%Y-%m-%d %H:%M:%S'),
                    "gender": str(athlete["gender"]),
...
def individual_date(client_id, range_start=None, range_end=None, warehouse_id=None, athlete_ids=None):
    data, shifts, range_start, range_end = all_athletes_data_shifts(
        client_id,
        range_start,
        range_end,
...
def individual_athletes(client_id, range_start=None, range_end=None, warehouse_id=None, athlete_ids=None):
...
def all_athletes(client_id, range_start=None, range_end=None, warehouse_id=None, athlete_ids=None):
...
def athletes(client_id, range_start=None, range_end=None, warehouse_id=None, athlete_ids=None):
...
def list_athletes(client_guid, query_str=None, is_active=None, hire_date_filter=None, job_function_filter=None, shift_filter=None, group_filter=None, sort_by=None, sort_dir=None, offset=None, limit=None, warehouse_id=None):
    query = db.db.session.query(db.IndustrialAthlete).options(
        joinedload(db.IndustrialAthlete.job_function),
        joinedload(db.IndustrialAthlete.shift)
    ).join(
        db.Client,
    ...
    count = count_athletes(client_guid, query_str, is_active, hire_date_filter, job_function_filter, shift_filter, group_filter, warehouse_id)

    result = {"total": count, "rows": athletes}

    return result

def count_athletes(client_guid, query_str=None, is_active=None, hire_date_filter=None, job_function_filter=None, shift_filter=None, group_filter=None, warehouse_id=None):
    query = db.db.session.query(sa.func.count("*")).select_from(db.IndustrialAthlete).join(
        db.Client,
    ).filter(
        db.Client.guid == client_guid
    )
...
def apply_athletes_list_filters(query, query_str=None, is_active=None, hire_date_filter=None, job_function_filter=None, shift_filter=None, group_filter=None):
    if query_str is not None and query_str != "":
        query = query.filter(
            sa.or_(db.IndustrialAthlete.external_id.like("%" + query_str + "%"), db.IndustrialAthlete.name.like("%" + query_str + "%"), db.IndustrialAthlete.first_name.like("%" + query_str + "%"), db.IndustrialAthlete.last_name.like("%" + query_str + "%"))
        )

    if is_active is not None:
        query = query.filter(
            db.IndustrialAthlete.available.is_(is_active)
        )
    ...
    if hire_date_filter is not None:
        ...
        query = query.filter(
            db.IndustrialAthlete.hire_date > date
        )
...
def group_count(client_guid, warehouse_id, query_str=None):
    ...
    unionlist = text("select  count(*) from (" + shift_query + " " + job_function_query + " " + group_query + ") as temp")
...
def get_group(id, warehouse_id):
    query = db.db.session.query(db.Group).filter(
        db.Group.id == id
    )
    data = [row for row in query]
    for row in data:
        job = {
            ...
            "name": row.name,
            "description": row.description,
            "athlete_count": count_group_athletes(row.id, "athlete_group", warehouse_id),
            "group_administrator": row.group_administrator,
...
def delete_shift(id, warehouse_id):
    if count_group_athletes(id, "shift", warehouse_id) == 0:
        try:
            ...
            return {
            'message': "No Shift found with this id"
            }
    else:
        return {
            'message': "You can not delete this Shift"
        }
...
def delete_jobfunction(id, warehouse_id):
    if count_group_athletes(id, "job_function", warehouse_id) == 0:
        try:
            ...
            return {
            'message': "No job function found with this id"
            }
    else:       
        return {
            'message': "You can not delete this Job function"
        }
    return {'message':'Please provide correct data'}
...
def add_note(request):
    ...
    client = db.db.session.query(
        db.Client).filter(
        db.Client.guid == client_guid).first()
...
def highlights(cid, range_start, range_end, warehouse_id):
    ...
    query = get_athlete_query(client_id=cid, range_start=range_start, range_end=range_end, warehouse_id=warehouse_id)
    ...
    for athlete_id in athletes_by_id:
        list.append(athlete_highlight_detail(
...
def has_group_data_in_table(group_list, table):
    return (
        db.db.session.query(
            sa.func.count(table.id)
        ).filter(
            sa.func.lower(table.name).in_(group_list)
        ).scalar() > 0
    )
...
def rankings(cid, range_start, range_end, warehouse_id, group_list=None):
    ...
    athlete_details_query = apply_warehouse_filter(athlete_details_query, warehouse_id).all()
    athlete_details = {}
    ...
    for athlete_detail in athlete_details_query:
        ...
        if show_athlete_identifier:
            athlete_details[athlete_detail.id]["athlete_identifier"] = athlete_detail.athlete_identifier
        if show_athlete_name:
            athlete_details[athlete_detail.id]["name"] = athlete_detail.name

        athlete_details[athlete_detail.id]["groups"] = get_group_name_list(athlete_detail)
        athlete_details[athlete_detail.id]["shift"] = get_rel_name(athlete_detail.shift)
        athlete_details[athlete_detail.id]["job_function"] = get_rel_name(athlete_detail.job_function)
    ...
    for athlete_id, athlete in athlete_details.iteritems():
        ...
        if show_athlete_identifier:
            athletes_by_id[athlete_id]["athlete_identifier"] = athlete["athlete_identifier"]
...
def athlete_highlight_detail(athlete, id, range_start, range_end):
    ...
    if safety_start_date > 0 and count > 1:
        most_improved_ss = ((avg_safety_score - safety_start_date) / float(safety_start_date)) * 100
...
def athlete_rank_detail(athlete, id, show_athlete_identifier, show_athlete_name):
    ...
    data = {
        ...
        "improve": can_improve(
            avg_max_flexion, avg_max_lateral_velocity, avg_twist_velocity, avg_safety_perc
        ),
        'groups': athlete['groups'],
        "shift": athlete["shift"],
        "job_function": athlete["job_function"],
    ...
    for item in data:
        if item.get(data_key, 0) != 0:
            if is_percent:
                res.append(
                    [
                        "{} %".format(item.get(key, 0)) if key == data_key else item.get(key, '')
                        for key in fieldnames
                    ]
                )
            else:
...
def export_athletes_csv(cid, range_start, range_end, warehouse_id, athletes=None):
    ...
    shifts = db.db.session.query(db.Shifts).join(db.Warehouse).filter(db.Warehouse.client_id == cid).all()
    range_start, range_end = get_date_range(cid, range_start, range_end)
    data = []
    ...
    data.append([
        "Name", "Facility", "Hire Date", "Job Function", "Shift", "Date",
        "Avg. Safety Score", "Avg. Max Flexion", "Avg Twist Velocity",
        "Max Moment", "Max Lateral Velocity", "Lift Rate", "Hours of Valid Lift Data",
...
def groups_athletes_csv(group_id,group_name):
    ...
    data = []
    data.append([
        "Id","Name","First Name","Last Name","Gender","Shift","Job Function","External ID","Hire Date","weight","height","Schedule","Warehouse","Client","Status",
        "Has Haptic","Athlete identifier","Db Create at","Db Modified at" 
...
def deactivate_athletes(filterdata):
    try:
        ...
        db.db.session.flush()        
        return {
            'message': 'Athletes deactivated successfully'
        }
    except NoResultFound:
        return {
        'message': "No athlete found with this filter criteria"
        }
    return {'message':'Please provide correct data'}
...
def activate_athletes(filterdata):
    try:
        ...
        db.db.session.flush()     
        return {
            'message': 'Athletes Updated succesffuly '
        }
    except NoResultFound:
        return {
        'message': "No athlete found with this filter critera"
        }
    return {'message':'Please provide correct data'}
...
def update_athletes_job_function(filterdata):
    try:
        ...
        query.update({"job_function_id": filterdata["job_function_id"]},synchronize_session='fetch')
        db.db.session.commit()
        db.db.session.flush()
        ...
        return {
            'message': 'Athletes Updated succesffuly ', 'status':status
        }
    except NoResultFound:
        return {
        'message': "No athlete found with this filter critera"
...
def update_athletes_groups(filterdata):
    try:
        ...
        return {
            'message': 'Athletes activated succesffuly '
        }
    except NoResultFound:
        return {
        'message': "No athlete found with this filter critera"
...
def apply_action_filters(query, filterdata):
    if 'jobFunctionFilter' in filterdata and filterdata['jobFunctionFilter'] is not None and len(filterdata['jobFunctionFilter']) > 0:
        query = query.filter(
            db.IndustrialAthlete.job_function_id.in_(filterdata["jobFunctionFilter"]))
        
    if 'shiftFilter' in filterdata and filterdata['shiftFilter'] is not None and len(filterdata['shiftFilter']) > 0:
        query = query.filter(db.IndustrialAthlete.shift_id.in_(filterdata["shiftFilter"]))

    if 'customGroupFilter' in filterdata and filterdata['customGroupFilter'] is not None and len(filterdata['customGroupFilter']) > 0:
...
def check_duplicate_group(warehouse_id, name):
    shifts_query = db.db.session.query(db.Shifts.name).filter(
        db.Shifts.warehouse_id == warehouse_id
    ).filter(
        db.Shifts.name == name
    )
    jobfunction_query = db.db.session.query(db.JobFunction.name).filter(
        db.JobFunction.warehouse_id == warehouse_id
    ).filter(
        db.JobFunction.name == name
    )
    athlete_group_query = db.db.session.query(db.Group.name).filter(
        db.Group.warehouse_id == warehouse_id
    ).filter(
        db.Group.name == name
    )
...
def create_shifts(request, user_id, client_id, warehouse_id):
    ...
    shift = db.Shifts(name = data.get('name'), 
                                  group_administrator = data.get('group_administrator'),
                                  description = data.get('description'),
                                  warehouse_id = warehouse_id)
    if  'shift_start' in data and data['shift_start'] is not None and data['shift_start'] != "":
        start_shift = datetime.datetime.strptime(data.get('shift_start'), '%H:%M:%S').time()
        shift.shift_start = start_shift
    else:
        shift.shift_start = datetime.datetime.now()

    if  'shift_end' in data and data['shift_end'] is not None and data['shift_end'] != "":
...
def create_job_functions(request, user_id, client_id, warehouse_id):
    ...
    if check_duplicate_group(warehouse_id,data.get('name')):
        return {'message':'A group already exists with same name'}
    job_function = db.JobFunction(name = data.get('name'), 
                                  group_administrator = data.get('group_administrator'),
    ...
    try:
        ...
        Activity.save_activity(user_id, client_id, 'job_function', job_function.id, 'CREATED')
...
def create_athlete(request, user_id, client_id, warehouse_id):
    ...
    if not data:
       return {'message': 'No input data provided'}
    exists = db.db.session.query(db.IndustrialAthlete.external_id).filter(
        db.IndustrialAthlete.client_id == client_id
    ).filter(
        db.IndustrialAthlete.external_id == data.get('external_id')
    ).first()
    if exists:
    ...
    athlete = db.IndustrialAthlete(first_name = data.get('first_name'), 
                                  last_name = data.get('last_name'),
                                  name = data.get('first_name') + ' ' + data.get('last_name'),
                                  gender = data.get('gender').lower(),
                                  height = data.get('height'),
                                  weight = data.get('weight'),
                                  hire_date = datetime.datetime.strptime(data.get("hire_date"), '%m-%d-%Y') if data.get("hire_date") else None,
    ...
    try:
        ...
        Activity.save_activity(user_id, client_id, 'industrial_athlete', athlete.id, 'CREATED')
...
def update_athlete(id, request, user_id, client_id, warehouse_id):
    ...
    if not data:
       return {'message': 'No input data provided'}
    try:
        athlete = db.db.session.query(db.IndustrialAthlete).filter(db.IndustrialAthlete.id==id).first()
        ...
        athlete.hire_date = datetime.datetime.strptime(data.get("hire_date"), '%m-%d-%Y') if data.get("hire_date") else None,
        ...
        Activity.save_activity(user_id, client_id, 'industrial_athlete', athlete.id, 'UPDATED')
...
def update_jobfunction(id, request, user_id, client_id, warehouse_id): 
    ...
    jobfunction = db.db.session.query(db.JobFunction).filter(db.JobFunction.id==id).first() 
    ...
    try: 
        ...
        db.db.session.commit()
        db.db.session.flush()
        query = db.db.session.query(db.IndustrialAthlete.id).filter(db.IndustrialAthlete.job_function_id == job_func_id)
        data = [row for row in query]
        athletes = []
        for row in data:
            athletes.append(row.id)
        reprocess_on_athlete_change(
            jobfunction.id,
            user_id,
            client_id,
            athletes
...
def upload_file(request, client_id, user_id, s3_bucket, s3_location):
    ...
    if request.method == 'POST' and  request.files['file'] !="" :
        file_name = str(datetime.datetime.now().time())+"-"+ request.files['file'].filename
        ...
        try:
            ...
            db.db.session.commit()
            db.db.session.flush()
            Activity.save_activity(user_id, client_id, 'import_record', importrecord.id, 'CREATED')
            return {"success":True,"importId":importrecord.id , 'total_records':total_records} 
        except SQLAlchemyError as e: 
            return {"success":False,'error':e.message}
...
def process_file(id, client_id, warehouse_id, user_id, s3_bucket, s3_location):
    import_record = db.db.session.query(ImportRecord).filter(ImportRecord.id==id).first()

    error, total_records, inserted_records = start_process(import_record.location, import_record.filename, s3_bucket, client_id, warehouse_id, id, user_id)
    import_errors = []
    if error > 0:
        import_errors = get_errors(id)
        return {'rows_with_error':error, 'total_records':total_records, 'errors':import_errors}
...
def import_status(id):
    ...
    query_athlete = db.db.session.query(sa.func.count("*")).select_from(db.IndustrialAthlete)
    query_athlete = query_athlete.filter(db.IndustrialAthlete.import_record_id==id)
    total_insert = query_athlete.scalar()
    
    query = text(""" SELECT count(distinct row_number) from import_error where import_record_id=:id """)
    ...
    if query_athlete or query_error:
        return { 'success':True, 'total_process': total, 'error':query_error,'insert': total_insert  }
    else:
...
def download_bulk_upload_csv_format():
    ...
    data.append([
        "*First Name","*Last Name","*Customer ID","*Gender (M/F/Other)","Weight (lbs)","Height (in)","*Known Prior Back Injuries (Y/N/Do Not Specifiy)"
        ,"Hire Date (mm/dd/yyyy)","*Job Function", "*Shift","",Note
    ])
    data.append(["","","","","","","","","","","","",""])
    data.append(["","","","","","","","","","","","Legend",""])
...
def trigger_reprocessing_api(athletes):
    ...
    try:
        resonse = requests.get('https://x3ird0fstb.execute-api.us-east-1.amazonaws.com/staging/reprocess?athlete_id_list='+athlete_ids+'&overwrite=true')
