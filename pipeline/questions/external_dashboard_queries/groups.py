...
def client_groups(client_guid, warehouse_id=None):
    ...
    results = []
    for group in query:
        results.append({"id": group.id, "name": group.name})
...
def custom_groups(client_guid, range_start=None, range_end=None, warehouse_id=None):
    ...
    query = (
        ...
        .group_by(
            db.IndustrialAthlete.id, db.Group.id, db.ProcessedFile.id, db.Fuse.has_flx
        )
        .order_by("id", "day")
    )
    ...
    client_groups = apply_warehouse_filter(
        client_groups, warehouse_id, column=db.Group.warehouse_id
    )
    ...
    for client_group in client_groups:
        groups_scores.setdefault(
            ...
            groups_scores.get(
                client_group.id,
                {"id": client_group.id, "name": client_group.name, "scores": []},
            ),
        )
...
def custom_group_score(
    client_guid, group_id, range_start=None, range_end=None, warehouse_id=None
):
    range_start, range_end = get_date_range(client_guid, range_start, range_end)
    query = (
        db.db.session.query(
    ...
    client_groups = (
        db.db.session.query(db.Group)
        .join(db.Warehouse, db.Client)
        .filter(db.Client.guid == client_guid, db.Group.id == group_id)
    )
