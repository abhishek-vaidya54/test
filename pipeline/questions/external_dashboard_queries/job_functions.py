...
from pipeline.db.questions.external_dashboard_queries.utils import generate_scores, generate_daily_scores, get_date_range, apply_warehouse_filter
...
def warehouse_job_functions(warehouse_id):
    ...
    ).join(db.Warehouse).filter(db.Warehouse.id == warehouse_id)
...
def client_job_functions(client_guid, warehouse_id=None):
    ...
    query = db.db.session.query(
        ...
        db.JobFunction.name.label("name"),
        db.JobFunction.color.label("color"),
    ).distinct(db.JobFunction.name).join(db.Warehouse, db.Client).filter(db.Client.guid == client_guid)

    query = apply_warehouse_filter(query, warehouse_id, column=db.JobFunction.warehouse_id)
...
def job_functions(client_guid, range_start=None, range_end=None, warehouse_id=None):
    ...
    client_job_functions = db.db.session.query(
        db.JobFunction
    ).join(
        db.Warehouse,
        db.Client
    ...
    client_job_functions = apply_warehouse_filter(
        ...
        warehouse_id,
        column=db.JobFunction.warehouse_id
    )
    ...
    for client_job_function in client_job_functions:
        job_functions_scores.setdefault(
            client_job_function.id, job_functions_scores.get(
...
def job_functions_by_score(client_guid, sorting_order, warehouse_id=None):
    ...
    query = db.db.session.query(
        ...
        db.ProcessedFile, db.Fuse, db.IndustrialAthlete, db.Warehouse, db.Client
    ).join(
...
def job_function_score(client_guid, jf_id, range_start=None, range_end=None, warehouse_id= None):
    range_start, range_end = get_date_range(client_guid, range_start, range_end)
