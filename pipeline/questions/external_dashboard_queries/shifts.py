...
def shifts(client_guid, range_start=None, range_end=None, warehouse_id= None):
    ...
    client_shifts = db.db.session.query(
        db.Shifts
    ).join(
    ...
    client_shifts = apply_warehouse_filter(client_shifts, warehouse_id, column=db.Shifts.warehouse_id)
    ...
    for client_shift in client_shifts:
        shifts_scores.setdefault(
            client_shift.id, shifts_scores.get(client_shift.id, {
                "id": client_shift.id,
...
def shift_score(client_guid, shift_id, range_start=None, range_end=None, warehouse_id= None):
    range_start, range_end = get_date_range(client_guid, range_start, range_end)
    query = db.db.session.query(
        sa.func.avg(db.Risk.safety_score).label('safety_score'),
        sa.func.avg(db.Risk.avg_twist_velocity).label('avg_twist_velocity'),
...
def list_shifts(client_guid, warehouse_id=None):
    ...
    for row in data:
        rows.append({"id": row.id, "name": row.name, "shift_start": row.shift_start.isoformat(), "shift_end": row.shift_end.isoformat()})
