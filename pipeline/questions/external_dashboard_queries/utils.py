...
def generate_scores(query, sorting_order):
    ...
    scores["safety_score"] = str(
        round(result["safety_score"] / result["total_rows"], Config.ROUNDING)
    )
    scores["avg_twist_velocity"] = str(
        round(result["avg_twist_velocity"] / result["total_rows"], Config.ROUNDING)
    )
    scores["max_flexion"] = str(
        round(result["max_flexion"] / result["total_rows"], Config.ROUNDING)
    )
    scores["max_moment"] = str(
        round(result["max_moment"] / result["total_rows"], Config.ROUNDING)
    )
    scores["max_lateral_velocity"] = str(
        round(result["max_lateral_velocity"] / result["total_rows"], Config.ROUNDING)
    )
    scores["lift_rate"] = str(
        round(result["lift_rate"] / result["total_rows"], Config.ROUNDING)
    )
...
def generate_daily_scores(query):
    ...
    for row in query:
        if row.id not in scores:
            scores[row.id] = {"id": row.id, "name": row.name, "scores": []}
                ...
                scores[v["id"]]["scores"].append(
                    {
                        ...
                        "safety_score": str(
                            round(v["safety_score"] / v["total_rows"], Config.ROUNDING)
                        ),
                        "max_flexion": str(
                            round(v["max_flexion"] / v["total_rows"], Config.ROUNDING)
                        ),
                        "avg_twist_velocity": str(
                            round(
                                v["avg_twist_velocity"] / v["total_rows"],
                                Config.ROUNDING,
                            )
                        ),
                        "max_moment": str(
                            round(v["max_moment"] / v["total_rows"], Config.ROUNDING)
                        ),
                        "max_lateral_velocity": str(
                            round(
                                v["max_lateral_velocity"] / v["total_rows"],
                                Config.ROUNDING,
                            )
                        ),
                        "lift_rate": str(
                            round(v["lift_rate"] / v["total_rows"], Config.ROUNDING)
                        ),
                        "day": str(v["day"]),
...
def get_latest_risk_query(client_id=None):
    query = (
        db.db.session.query(db.Risk.start_time)
        .join(
            db.ProcessedFile,
            db.Fuse,
        ...
        .filter(
            db.ProcessedFile.version == Config.ALGO_VERSION, db.Client.id == client_id
        )
        .order_by(desc(db.Risk.start_time))
        .limit(1)
    )
...
def apply_warehouse_filter(
    query, warehouse_id, column=db.IndustrialAthlete.warehouse_id
):
    if warehouse_id:
