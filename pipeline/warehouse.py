...
class Warehouse(db.Model):
    ...
    client = db.relationship("Client", foreign_keys=client_id, backref="warehouses")

    name = db.Column(db.String(255), nullable=False)
    location = db.Column(db.String(500), nullable=True)
    prefered_timezone = db.Column(db.String(100), server_default="UTC", nullable=False)
    ...
    update_engagement = db.Column(db.Integer, default=1, nullable=False)
...
class Masquerade(db.Model):
    ...
    original_warehouse_id = db.Column(
        db.Integer, ForeignKey("warehouse.id"), nullable=True
    )
    original_warehouse = db.relationship(
        "Warehouse", foreign_keys=original_warehouse_id, backref="masqurades"
    )
    ...
    masquerade_warehouse = db.relationship(
        "Warehouse", foreign_keys=masquerade_warehouse_id, backref="masquerade"
    )
