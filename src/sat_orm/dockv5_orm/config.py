...
def insert_or_update(session, data):
    ...
    dock_in_table = session.query(Config).filter_by(
        dock_id=data['dock_id']).first()
    if dock_in_table:
...
def validate_before_insert(mapper, connection, target):
    ...
    if "warehouse_id" in params_input:
        is_valid = ia_utils.is_valid_warehouse(
            connection, params_input.get(
                "warehouse_id", ""), params_input.get("client_id", "")
        )
        if not is_valid:
            errors.append(
                build_error("warehouseId",
                            constants.INVALID_WAREHOUSE_CLIENT_MESSAGE)
            )
