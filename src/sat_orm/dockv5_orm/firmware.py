...
class Firmware(Base):
    ...
    device_type = relationship("DeviceType", back_populates="firmwares")
...
#     if not is_valid:
#         errors.append(build_error("s3_url", constants.EMPTY_STRING_ERROR_MESSAGE))
...
#         is_valid = fu.is_non_empty_string(params_input.get("s3_url", ""))
#         if not is_valid:
#             errors.append(build_error("s3_url", constants.EMPTY_STRING_ERROR_MESSAGE))
