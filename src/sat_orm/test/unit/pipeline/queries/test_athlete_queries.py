...
def test_get_ia_by_external_id_valid(get_external_admin_user, test_session):
    ...
    result = athlete_queries.get_ia_by_external_id(
        test_session, ia.external_id, ia.warehouse_id
    )
    assert result


def test_get_ia_by_external_id_invalid(
    get_external_admin_user, invalid_id, test_session
):
    """
    ...
    ia = test_session.query(IndustrialAthlete).first()
    result = athlete_queries.get_ia_by_external_id(
        test_session, invalid_id, ia.warehouse_id
    )
    assert result == None
