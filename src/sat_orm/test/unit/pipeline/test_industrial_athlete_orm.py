...
def test_industrial_athlete_client_relationship(get_external_admin_user, test_session):
    ...
    assert (
        athlete
        == test_session.query(ia.IndustrialAthlete).filter_by(id=athlete.id).first()
    )
...
@pytest.mark.relationships
def test_industrial_athlete_warehouse_relationship(
    get_external_admin_user, test_session
):
...
@pytest.mark.relationships
def test_industrial_athlete_job_function_relationship(
    get_external_admin_user, test_session
):
...
@pytest.mark.test_select
def test_dockv5_getAthletes_select_by_client_warehouse_not_terminated(
    get_external_admin_user, test_session
):
    """
    verify all client_id and warehouse_id values from our result match what was queried for
    """
    result = ia.dockv5_getAthletes_select_by_client_warehouse_not_terminated(
        test_session,
        get_external_admin_user.client_id,
        get_external_admin_user.warehouses[0].warehouse_id,
    )
    assert all(
        [r.client_id == client_id and r.warehouse_id == warehouse_id for r in result]
    )
...
def test_dockv5_getEngagement_select_by_id(get_external_admin_user, test_session):
    ...
    """
    result = ia.dockv5_getEngagement_select_by_id(
        test_session, get_external_admin_user.id
    )
    assert result.id == get_external_admin_user.id


@pytest.mark.test_select
def test_dockv5_getUpdatedAthletes_select_group_id(
    get_external_admin_user, test_session
):
    """
    verify that athletes selected by the group_ids of the result contain the correct client_id and warehouse_id
    """
    result = ia.dockv5_getUpdatedAthletes_select_group_id(
        test_session,
        get_external_admin_user.client_id,
        get_external_admin_user.warehouses[0].warehouse_id,
    )
    ...
    validation_result = (
        test_session.query(ia.IndustrialAthlete)
        .filter(ia.IndustrialAthlete.group_id in group_ids)
        .all()
    )
    assert all(
        [
            r.client_id == client_id and r.warehouse_id == warehouse_id
...
def test_dockv5_getUpdatedAthletes_select_id(get_external_admin_user, test_session):
    ...
    """
    result = ia.dockv5_getUpdatedAthletes_select_id(
        test_session,
        get_external_admin_user.client_id,
        get_external_admin_user.warehouses[0].warehouse_id,
    )
    validation_result = (
        test_session.query(ia.IndustrialAthlete)
        .filter(ia.IndustrialAthlete.id in [r.id for r in result])
        .all()
    )
    assert all(
        [
...
@pytest.mark.test_select
def test_dockv5_getUpdatedAthletes_select_by_db_modified(
    get_external_admin_user, test_session
):
    """
...
@pytest.mark.test_select
def test_dockv5_getUpdatedAthletes_select_by_client_warehouse_db_modified(
    get_external_admin_user, test_session
):
    """
    verify that athletes selected by the timestamp have db_modified_at >= the tiemstamp that was queried for
    ...
    timestamp = "2018-01-01"
    dt = datetime.datetime.strptime(timestamp, "%Y-%m-%d")
    result = ia.dockv5_getUpdatedAthletes_select_by_client_warehouse_db_modified(
        test_session, client_id, warehouse_id, timestamp
    )
    assert all(
        [
            r.db_modified_at >= dt
