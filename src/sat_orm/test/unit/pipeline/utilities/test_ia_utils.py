...
def test_is_valid_ia_first_name_valid(valid_ia_put_body, test_session):
    ...
    Output:
        True
    """
    result = ia_utils.is_valid_ia_first_last_name(
        test_session,
        valid_ia_put_body["firstName"],
        "First Name",
        valid_ia_put_body["clientId"],
    )
    assert result


def test_is_valid_ia_first_name_invalid(
    invalid_athletes_put_body_invalid_first_name, test_session
):
    """
    ...
    result = ia_utils.is_valid_ia_first_last_name(
        test_session,
        invalid_athletes_put_body_invalid_first_name["firstName"],
        "First Name",
        invalid_athletes_put_body_invalid_first_name["clientId"],
    )
...
def test_is_valid_ia_last_name_invalid(
    invalid_athletes_put_body_invalid_last_name, test_session
):
    """
    Checks if is_valid_ia_first_last_name method is returning False or not
    Input:
    ...
    result = ia_utils.is_valid_ia_first_last_name(
        test_session,
        invalid_athletes_put_body_invalid_last_name["lastName"],
        "Last Name",
        invalid_athletes_put_body_invalid_last_name["clientId"],
    )
    assert result[0] == False
...
def test_is_valid_external_id_invalid(
    get_external_admin_user, invalid_string_space_front, test_session
):
    """
    ...
    """
    ia = test_session.query(IndustrialAthlete).first()
    result = ia_utils.is_valid_external_id(
        test_session, ia, invalid_string_space_front, ia.id
    )
    assert result
...
def test_is_valid_warehouse_valid(get_external_admin_user, test_session):
    ...
    result = ia_utils.is_valid_warehouse(
        test_session, get_external_admin_user.warehouses[0].warehouse_id
    )
    assert result
...
def test_is_valid_shift_invalid_shift_id(
    invalid_id, get_external_admin_user, test_session
):
    """
...
def test_is_valid_shift_invalid_warehouse_id(
    invalid_id, get_external_admin_user, test_session
):
    """
    Checks if is_valid_shift method is returning False or not
...
def test_is_valid_job_function_valid(get_external_admin_user, test_session):
    ...
    """
    ia = test_session.query(IndustrialAthlete).first()
    result = ia_utils.is_valid_job_function(
        test_session, ia.job_function_id, ia.warehouse_id
    )
    assert result


def test_is_valid_job_function_invalid_job_function_id(
    invalid_id, get_external_admin_user, test_session
):
    """
...
def test_is_valid_job_function_invalid_warehouse_id(
    invalid_id, get_external_admin_user, test_session
):
    """
    Checks if is_valid_job_function method is returning False or not
    ...
    ia = test_session.query(IndustrialAthlete).first()
    result = ia_utils.is_valid_job_function(
        test_session, ia.job_function_id, invalid_id
    )
    assert result == False
