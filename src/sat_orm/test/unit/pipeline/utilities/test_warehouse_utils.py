...
def test_is_valid_lat_long_direction_valid():
    ...
    checks if function returns true for correct params
    """
    res = warehouse_utils.is_valid_lat_long_direction(
...
def test_is_valid_warehouse_valid(test_session, get_warehouse_from_db):
    ...
    """
    warehouse = get_warehouse_from_db
    res = warehouse_utils.is_valid_warehouse(
        test_session, warehouse.id, warehouse.client_id
    )
    assert res


def test_is_valid_warehouse_invalid_warehouse_id(
    test_session, invalid_int, get_warehouse_from_db
):
    """
    checks if function returns false for invalid warehouse id
    """
    warehouse = get_warehouse_from_db
    res = warehouse_utils.is_valid_warehouse(
        test_session, invalid_int, warehouse.client_id
    )
    assert res is False


def test_is_valid_warehouse_invalid_client_id(
    test_session, invalid_int, get_warehouse_from_db
):
    """
